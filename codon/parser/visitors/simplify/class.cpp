...
std::shared_ptr<FunctionStmt> generateApply(SimplifyVisitor& visitor, std::shared_ptr<std::vector<StmtPtr>> preamble, std::shared_ptr<SimplifyContext> ctx, ClassStmt *cls, vector<FunctionStmt*> actionStmts) {
  vector<StmtPtr> suites;
  std::vector<Param> fargs;
  auto ret = std::make_shared<IdExpr>("NoneType");
  Attr attr;
  attr.set("autogenerated");
  LOG("generating apply method for class {}", cls->name);
  fargs.emplace_back(Param{"receiver", std::make_shared<IdExpr>("u64")});
  fargs.emplace_back(Param{"first_receiver", std::make_shared<IdExpr>("u64")});
  fargs.emplace_back(Param{"action", std::make_shared<IdExpr>("u64")});

//   auto mainSmts = SimplifyVisitor(ctx, preamble)
//                         .transform(codon::ast::parseCode(ctx->cache, ctx->getFilename(), 
//                         fmt::format(R"(
// contract = {}(receiver, first_receiver, action)
// dec = new_action_decoder();
//         )"
//     , cls->name)));
  suites.push_back(std::make_shared<ImportStmt>(std::make_shared<IdExpr>("action"), std::make_shared<IdExpr>("new_action_decoder")));
  // suites.push_back(mainSmts);
  suites.push_back(std::make_shared<AssignStmt>(std::make_shared<IdExpr>("contract"),
    std::make_shared<CallExpr>(make_shared<IdExpr>(cls->name),
        std::make_shared<IdExpr>("receiver"),
        std::make_shared<IdExpr>("first_receiver"),
        std::make_shared<IdExpr>("action")
  )));
  ...
  for (auto fn: actionStmts) {
    ...
    for (auto &a: fn->args) {
      ...
      auto id = fmt::format("arg{}", i);
      i++;
      args.push_back(std::make_shared<IdExpr>(id));
      stmts.push_back(std::make_shared<AssignStmt>(std::make_shared<IdExpr>(id),
        std::make_shared<CallExpr>(
          std::make_shared<DotExpr>(std::make_shared<IdExpr>("dec"), "unpack"),
          a.type)));
    }
    LOG("++++++fn->decorators{}", fn->decorators);
    auto actionName = extract_attr_name(Attr::Action, fn->decorators);

    auto call = std::make_shared<DotExpr>(std::make_shared<IdExpr>("contract"), fn->name);
...
void SimplifyVisitor::visit(ClassStmt *stmt) {
  ...
  try {
    ...
    if (is_contract || is_table) {
      LOG("+++++++class name: {}", name);
    }

    if (is_table) {
      ABIGenerator::instance().addTable(stmt, args);
    }
    ...
    auto methods = getClassMethods(stmt->suite);
    // Add class methods
    for (const auto &sp : methods) {
      if (!sp) {
        continue;
      }
      auto fn = sp->getFunction();
      if (!fn) {
        continue;
      }
      ...
      if (sp.get() != autoDeducedInit.second) {
        if (is_contract) {
          if (ABIGenerator::instance().addAction(fn)) {
            actionStmts.push_back(fn);
          }
        }
      }
    }
    ...
    if (is_contract) {
      auto applyFn = generateApply(*this, preamble, ctx, stmt, actionStmts);
      fnStmts.push_back(transform(applyFn));
    }
